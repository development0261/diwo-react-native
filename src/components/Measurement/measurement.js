        ...
        if(this.state.count==1){
          ...
          .then((responseJson) => {
            if(responseJson.status==200){
              ...
              console.log(month);
              console.log(responseJson.kapital_data);
              this.setState({dataSocialKapital:responseJson.kapital_data,monthGraph:month,totalGraph:arr});
            }else{
              alert("Something Bad happen");
            }
          }).catch((error) =>{
      ...
      <View style={styles.container}>         
        ...
        <View style={{flex:1,marginTop:10,marginBottom:10}}>
          ...
          <View style={{alignItems:'center',marginTop:15}}>
            <Text style={{fontSize: width * 0.04}}>{Text_EN.Text_en.measurement_workjoy_title}</Text>
            {this.state.dataSource.length!=0?<FlatList
              style={{marginTop:5}}
              data={this.state.dataSource}
              showsVerticleScrollIndicator={false}
              renderItem={({item}) =><View style={{flexDirection:'row',paddingBottom:10}}>
                <Text style={{fontSize: width * 0.04,marginRight:10}}>Uge {item.week} </Text>
                  {item.review=="Bad"?<Image
                    ...
                    />:null}
                <Text style={{fontSize: width * 0.04,marginLeft:10,width:'60%'}}>Kommentar : {item.comments}</Text>
              </View>}
              keyExtractor={({id}, index) => id}
            />:<Text style={styles.nosocialKaptial}>No data found</Text>}
          </View>
          <View style={{marginTop:15}}>
            <Text style={{fontSize: width * 0.04,alignSelf:'center'}}>{Text_EN.Text_en.measurement_socialkaptial_title}</Text>
            ...
            {/* <Text>length:{this.state.dataSocialKapital.length}</Text> */}
                    ...
                    <Card borderRadius={10}>
                      <View style={{flexDirection:'row'}}>
                        <View style={{justifyContent:'center'}}>
                          <Text style={{fontSize: width * 0.03,fontWeight:'bold'}}>{months[item.review_date.substring(5,7).replace(/^0+/, '')-1]} </Text>
                        </View>
                        <View  style={{borderLeftWidth: 1,borderLeftColor: '#00a1ff',margin:5,marginLeft:4}}></View>
                        <View>
                          ...
                          <Text style={{fontSize: width * 0.035}}>{Text_EN.Text_en.cooperation}: <Text style={{fontWeight:'bold'}}>{item.cooperation}%</Text></Text>
                          <Text style={{fontSize: width * 0.035}}>{Text_EN.Text_en.trust}: <Text style={{fontWeight:'bold'}}>{item.trust}% </Text></Text>
                          <Text style={{fontSize: width * 0.035}}>{Text_EN.Text_en.justice}: <Text style={{fontWeight:'bold'}}>{item.justice}%</Text></Text>
                          <Text style={{fontSize: width * 0.035,fontWeight:'bold',borderTopWidth:1}}>{Text_EN.Text_en.total}: {item.trust + item.cooperation + item.justice}% </Text>
                        </View>
                      </View>
                    </Card>
                  </View>}
              ...
              <View style={{width:'40%',marginLeft:10,marginTop:20}}>
                ...
                {this.state.dataSocialKapital.length>1?<LineChart
                  ...
                  width={width * 0.34} // from react-native
                  height={height * 0.28}
                  chartConfig={{
